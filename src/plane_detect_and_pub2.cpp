...
void replaceZeroDepth_and_generatePointCloud(cv::Mat& depthImage, const Eigen::Isometry3f &TransMatrix, const myPointCloud::Ptr Cloud) 
{
    ...
    for(int i = 0; i < height; ++i)
    {
        for(int j = 0; j < width; ++j)
        {
            ...
            Eigen::Vector3f pointWorld = TransMatrix * point;
            pcl::PointXYZ p;
            p.x = pointWorld[0];
            p.y = pointWorld[1];
            p.z = pointWorld[2];
...
private:
    void imagCallback(const sensor_msgs::ImageConstPtr& depthImageMsg)
    {
        ...
        T_w_l.pose.orientation.x = qx;
        T_w_l.pose.orientation.y = qy;
        T_w_l.pose.orientation.z = qz;
        T_w_l.pose.orientation.w = qw;
        ...
        replaceZeroDepth_and_generatePointCloud(imgDepth, Twc, Cloud);
